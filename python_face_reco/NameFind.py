...
#Verification of the last ID in Nomes.txt (last_string) - CREATED BY IGOR
...
with open('Nomes.txt') as f:
    ...
    if file_is_empty('Nomes.txt'):
        ...
        for s in string_last.split(): 
            if s.isdigit():
                last_string = int(s) 
                print("A base possui: " + str(last_string - 1) + " " + "pessoa(s)" )

def ID2Name(ID, conf):
    
    if ID>=1 and ID<=last_string:
        NameString =  Names[ID-1] + ", Confianca: " + (str(round(conf)) )                                # Find the Name using the index of the ID
    else:
        NameString = " Rosto nao reconhecido "  # Find the Name using the index of the ID
...
def AddName():
    ...
    print ("Nome salvo com Id: " + str(ID))
    Info.close()
    return ID
